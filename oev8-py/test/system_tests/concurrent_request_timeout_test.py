...
from pytest import mark, fixture, fail  # type:ignore
...
logger = logging.getLogger('test.system_tests.concurrent_request_timeout_test')
...
async def tbody_sleep(
    ...
    await tcp_cmd_client.sleep(5)  # 설정파일에 지정된 값보다 조금 크게.
...
async def tbody_ping(
    ...
    with sw:
        ...
        try:
            result = await tcp_cmd_client.ping()

            await queue.put('PONG')
            logger.debug('GOT PONG: %s', result)

            #
            fail('expected a RequestTimedOut')
        except RequestTimedOut:
            await queue.put('no-PONG')
            logger.debug('Got RequestTimedOut, OK',
                         exc_info=True)
...
async def tbody_concurrent_client_1st_sleep(
    ...
    assert l == ['SLEEP', 'PING', 'SLEPT', 'no-PONG'] \
        or l == ['SLEEP', 'PING', 'no-PONG', 'SLEPT']
