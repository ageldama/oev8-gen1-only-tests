...
logger = logging.getLogger('test.system_tests.recover_from_only_snapshot_test')
...
def test_recover_from_only_snapshot(
        oev8_sh_cmd,
        ...
        test_config_path,
        #
        journal_kafka_topic_resetter,
        journal_kafka_depth_getter,
):
    ...
    - Journals에 기록도 모두 날려서, 오직 스냅샷만으로 복구.
    '''
    # testing steps:
    ...
    # journal stream 다시 생성해서, 비우기.
    journal_kafka_topic_resetter()

    # 복구 직전 저널 카운트 == 0.
    assert journal_kafka_depth_getter() == 0
