"""oev8.svcs.trading.TradingService.order_market_sell() + FoK 테스트."""
...
def test_order_market_sell_with_fok_cancels_with_no_match(
        trading_service: TradingService,
        event_writer_mock
):
    """order_market_sell + FoK 테스트: 매칭이 모두 되지 않았다면 아무 것도 하지 않고 취소."""
    ...
    # balance-check?
    assert balance_service.get(BalanceType.BALANCE, cust_c, curr) == 0
...
def test_order_market_sell_with_fok_with_partial_match(
        sell_fixture,
        event_writer_mock
):
    """order_market_sell + FoK / Partial match 테스트."""
    # pylint: disable=redefined-outer-name
    ...
    oid = trading_service.order_market_sell(
        trd_id, cust_c, ItemQty(100), option=OrderOption.FILL_OR_KILL)
    ...
    # item-count: 모두 되돌려 받음.
    assert item_count_service.get(cust_c, trd_id) == 100
    assert item_count_service.get(SERVICE_CUSTOMER, trd_id) == 0
    ...
    # event_writer.
    event_writer_mock.on_item_count_xfer_to.assert_not_called()
    event_writer_mock.on_trading_market_sell_order.assert_called_once()
    event_writer_mock.on_trading_order_matched.assert_not_called()
    event_writer_mock.on_trading_order_cancelled.assert_called_once()
    event_writer_mock.on_trading_order_cancelled.assert_has_calls([
        call(trd_id=trd_id, ord_id=oid, cust_id=cust_c, price=ANY,
             qty=100, remaining_qty=100,
             why=TradingOrderCancelCauseFoK())
    ])


def test_order_market_sell_with_fok_with_fulfilled(
        sell_fixture,
        event_writer_mock
):
    """order_market_sell + FoK / Fulfilled 테스트."""
    # pylint: disable=redefined-outer-name
    ...
    oid = trading_service.order_market_sell(
        trd_id, cust_c, ItemQty(3), option=OrderOption.FILL_OR_KILL)
